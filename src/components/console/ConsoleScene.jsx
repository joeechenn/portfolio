...
import { HomeScreen, SoftwareMenu } from './Screens'
...
function HtmlSurface({ width, height, children, ...props }) {
  const { size, invalidate } = useThree()
  const scale = pixelsPerUnit(size)
  // Html mounts through a separate React root; request a frame after its DOM commits.
  return <Html transform distanceFactor={400 / scale} {...props}><div ref={() => invalidate()} style={{ width: width * scale, height: height * scale }}>{children}</div></Html>
}
...
function Lid() {
  ...
  return (
    <group name="lid" rotation={[25 * Math.PI / 180, 0, 0]}>
      ...
      <HtmlSurface width={DISPLAY.top[0]} height={DISPLAY.top[1] + extra} position={[0, 1.97 + extra / 2, 0.175]}><HomeScreen /></HtmlSurface>
      ...
      <HardwareLabel position={[3.4, 1.22, 0.17]}>3D</HardwareLabel>
      <HardwareLabel position={[3.4, 0.92, 0.17]}>OFF</HardwareLabel>
...
function Base() {
  ...
  return (
    <group name="base">
      ...
      <HtmlSurface width={DISPLAY.bottom[0] + wider} height={DISPLAY.bottom[1] + extra} position={[0, -1.75 - extra / 2, 0.275]}><SoftwareMenu /></HtmlSurface>
      ...
      <Panel width={0.3 * dpadScale} height={0.99 * dpadScale} depth={0.09} radius={0.035} position={[-controlX, -2.44, 0.23]} color="#111316" />
      <Panel width={0.99 * dpadScale} height={0.3 * dpadScale} depth={0.092} radius={0.035} position={[-controlX, -2.44, 0.23]} color="#111316" />
      {[[0, 0.3, 0.015, 0.15], [0, -0.3, 0.015, 0.15], [-0.3, 0, 0.15, 0.015], [0.3, 0, 0.15, 0.015]].map(([x, y, w, h], index) => (
        <Panel key={index} width={w * dpadScale} height={h * dpadScale} depth={0.005} radius={0.005} position={[-controlX + x * dpadScale, -2.44 + y * dpadScale, 0.345]} color="#62666b" />
      ))}
      {[['X', faceX, -0.66], ['Y', faceX - faceSpacing, -1.05], ['A', faceX + faceSpacing, -1.05], ['B', faceX, -1.45]].map(([label, x, y]) => (
        <group key={label}>
          ...
          <HardwareLabel width={0.28} height={0.28} position={[x, y, 0.35]}><span className="face-letter">{label}</span></HardwareLabel>
        </group>
      ))}
      {[['SELECT', -1.68], ['⌂ HOME', 0], ['START', 1.68]].map(([label, x]) => (
        <group key={label}>
          ...
          <HardwareLabel width={1.3} position={[x, -3.58 - extra, 0.23]}>{label}</HardwareLabel>
        </group>
      ))}
      <Panel width={0.24} height={0.24} depth={0.04} radius={0.05} position={[3.17, -3.17 - extra, 0.2]} color="#15171a" />
      <HardwareLabel width={0.2} height={0.2} position={[3.17, -3.17 - extra, 0.27]}>⏻</HardwareLabel>
      <HardwareLabel width={0.7} position={[3.7, -3.17 - extra, 0.23]}>POWER</HardwareLabel>
...
function Hardware() {
  const { size } = useThree()
  return (
    <group rotation={[(size.width < 600 ? -12 : -17) * Math.PI / 180, 0, 0]}>
      <Lid />
      <Base />
...
export default function ConsoleScene({ fallback }) {
  return (
    <Canvas orthographic camera={{ position: [0, 0, 20], zoom: 90, near: 0.1, far: 50 }} frameloop="demand" dpr={[1, 2]} gl={{ antialias: true, alpha: true }} fallback={fallback}>
      <Camera />
